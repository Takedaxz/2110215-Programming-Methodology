...
public class Player {
    ...
    public String useWeapon(BaseWeapon item, Player opponent) {
        int distance = Math.abs(this.getPos()-opponent.getPos());
        if(item.getRange()>=distance){
    ...
    public void beingAttack(int att) {
        this.setHp(getHp()-(Math.max(0,att-calculateDef())));
        decreaseArmorDur();
    }
    ...
    public String useNonWeaponItem(BaseItem item) {
        ...
        if(item instanceof BaseArmor baseArmor){
            s.add(wear(baseArmor));
        }
        if(item instanceof Healable healable){
            s.add(useHeal(healable));
        }
        if(item instanceof AttBuffable attBuffable){
