...
public class BookListPane extends VBox {
    ...
    public Button newBookButton() {
        ...
        add.setOnMouseClicked(e -> Goto.addNewBookPage());
        return add;
    }
    ...
    public void setSearchedBooks(ArrayList<Book> searchedBooks) {
        ...
        for (int i = 0; i < searchedBooks.size(); i++) {
            BookPane tmp = new BookPane(searchedBooks.get(i));

            if (i % 2 == 0) {
                tmp.setBackground(Background.fill(Color.WHITE));
            } else {
                tmp.setBackground(Background.fill(Color.color(0, 0, 0, 0.05)));
            }
